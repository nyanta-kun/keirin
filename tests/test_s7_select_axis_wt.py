...
def test_overlap_of_three_picks_top2_by_top3():
    ...
    result = s7_select_axis(win_probs, top3_probs)
...
def test_returns_none_when_no_overlap():
    ...
    top3_probs = {1: 0.90, 2: 0.60, 3: 0.55, 4: 0.10, 5: 0.05, 6: 0.05, 7: 0.05}
    assert s7_select_axis(win_probs, top3_probs) is None


def test_returns_none_when_insufficient_entries():
    assert s7_select_axis({1: 0.5, 2: 0.3}, {1: 0.5, 2: 0.3}) is None


def test_returns_none_when_empty():
    assert s7_select_axis({}, {1: 0.5, 2: 0.3, 3: 0.2}) is None
    assert s7_select_axis({1: 0.5, 2: 0.3, 3: 0.2}, {}) is None
