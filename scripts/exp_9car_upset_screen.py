...
### (A)−(B) の母集団（A_THR=500 / B_THR=50・カットは決定期で固定して一度きり評価）

| 構成 | 決定期 ratio | 評価期 n | 評価期 500倍+率 | 評価期 ratio | 帯ROI |
|---|---|---|---|---|---|
| 全件 | 0.923 | 1,486 | 15.07% | 0.845 | 63.4% |
| B除外50% のみ | 0.994 | 701 | 16.41% | 0.884 | 66.3% |
| **A上位30% のみ** | 1.004 | 528 | 18.56% | **1.000** | **75.0%** |
| **A上位30% ∧ B除外50%** | 1.083 | 373 | 20.11% | **1.062** | **79.6%** |
| A上位20% ∧ B除外50% | 1.088 | 259 | 20.46% | 1.071 | 80.3% |

**決定期と評価期で符号・幅とも一致**し、A を絞るほど・B を外すほど単調に改善する
（単一セルの当たりではない）。A_THR=300 では B の寄与が相対的に大きく、
A_THR=500 では **A の寄与が主**になる。B_THR は 50倍が最良（30倍にすると悪化）。

### ⚠️ ただし統計的に有意ではない（標本不足）

    A上位30% ∧ B除外50%   Δratio(選別−全体) = +0.0719  95%CI [-0.0604, +0.2028]
    A上位20% ∧ B除外50%   Δratio             = +0.1107  95%CI [-0.0643, +0.2842]
    月次で全体を上回った月  19/32 (59%) / 20/31 (65%)

**効果量は 7車で有意だった同じ現象と同じ大きさ**
（[[keirin_highpay_tail_mispricing_2026_08_08]]: 7車 Δratio +0.107・n=23,000 で有意）
なのに、9車は 4,738R しかないため CI が5倍広い。
...
def _load(n_entries: int) -> list[dict]:
    """レース単位の (オッズ非依存の特徴, 決着オッズ, 帯ごとの市場含意) を返す。

    ⚠️ 絞り込みは `wt_races` への JOIN で書く。`race_key IN (SELECT ...)` にすると
    `wt_odds`（2,200万行）のプランが崩れる。
    """
...
def _features(ents: list) -> dict:
    ...
    for e in ents:
        lines[e["line_group"] if e["line_group"] is not None else f"solo{e['frame_no']}"].append(e)
    ...
    f = {
        ...
        # ライン構成 — 競輪はライン戦なので「隊列の作り」が決着形を決める
    ...
    if all(x is not None for x in pw) and all(x is not None for x in p3):
        ...
        f.update(pw_max=np.nan, pw_gap12=np.nan, pw_entropy=np.nan,
                 p3_max=np.nan, p3_std=np.nan, p3_entropy=np.nan)
    return f
...
def _enc(v) -> int:
    return abs(hash(str(v))) % 1000 if v is not None else -1
...
def _strata(rows: list[dict], score: np.ndarray, y: np.ndarray,
            implied: np.ndarray, label: str, n_bin: int = 5) -> None:
    ok = ~np.isnan(score)
    if ok.sum() < 200:
        print(f"  {label}: 標本不足"); return
    ...
    for i in range(n_bin):
        m = (s >= edges[i]) & (s <= edges[i + 1] if i == n_bin - 1 else s < edges[i + 1])
        if m.sum() < 30:
            continue
        r = yy[m].mean() / im[m].mean() if im[m].mean() else float("nan")
...
def main() -> None:
    ...
    ap.add_argument("--best-a", type=float, default=0.3, help="月次を見る採用候補のA上位割合")
    ...
    cols = [c for c in rows[0]
            if c not in ("race_key", "date", "win_odds", "board", "A", "B", "impA", "impB")]
    ...
    _strata(rows, sA, yA, impA, f"A スコア5分位（>={args.a_thr:.0f}倍）")
    print(f"\n=== (B) 人気決着の予測  AUC={_auc(yB, sB):.4f} ===")
    _strata(rows, sB, yB, impB, f"B スコア5分位（<={args.b_thr:.0f}倍）")

    # 単一特徴のベースライン。45特徴モデルが rp_std 単体に負けた前例があるため必ず併記する
    print("\n=== 単一特徴ベースライン（Aの上位半分での 実測 / ratio）===")
    ok = ~np.isnan(sA)
    ...
    if dev.sum() >= 400 and con.sum() >= 400:
        ...
        deltas = []
        for _ in range(1000):
            idx = rng.choice(len(days_l), size=len(days_l), replace=True)
            b = [i for j in idx for i in by_day[days_l[j]]]
...
def _population(rows, sA, sB, yA, yB, impA, impB, ok, args) -> None:
    print("\n=== 母集団 (A高) − (B高) ===")
