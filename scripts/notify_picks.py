...
def main():
    ...
    # 新S1（6車三連単）は 2026-07-17 全廃（現行ペーパーランクは S1 / 7SS+・7SS・7S(7車)
    # / 9SS+・9SS・9S(9車) のみ。表示ランク名はkiseki Web(page.tsx)と一致させること）
    total = ss_n
    ...
    # ── ヘッダー送信 ──────────────────────────────────────────────────────────
    # 2026-07-27〜: 現行ランクは S1／7SS+・7SS・7S(7車)／9SS+・9SS・9S(9車)（S2/S3は全廃）。
    # 表示ランク名はWeb（kiseki frontend/src/app/keirin/page.tsx RANK_STYLE）と一致させること。
    header = (
        f"🚲 **{title_label} {target_date}**\n"
        f"※S1／7SS+・7SS・7S／9SS+・9SS・9S は発走15分前に個別通知"
    )
