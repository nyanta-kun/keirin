#!/usr/bin/env python3
"""7A(RANK_7A) の全期間honest再構築（quarterly walk-forwardモデル使用）。
...
def wipe_rows_pg(date_from: str, date_to: str, dry_run: bool) -> None:
    ...
    with get_connection() as conn:
        ...
        print(f"[rebuild-7a-pg] 既存 #7A 行（{date_from}〜{date_to}）: {n}件 → 削除"
...
def insert_rows_pg(rows: list[dict], dry_run: bool) -> None:
    ...
    with get_connection() as conn:
        conn.executemany(
            ...
            rows_ins)
        conn.commit()
    print(f"[rebuild-7a-pg] {len(rows)}件 書き込み完了（VPS PG）")
...
def main() -> None:
    ...
    for date_from, date_to, eval_model, win_model in windows:
        print(f"\n[rebuild-7a-pg] {date_from}〜{date_to}  eval={eval_model} win={win_model}", flush=True)
    ...
    if args.dry_run:
        print("[rebuild-7a-pg] DRY RUN（書き込みなし）")
