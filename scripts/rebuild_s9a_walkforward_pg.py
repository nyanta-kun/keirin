#!/usr/bin/env python3
"""9A(NINE_9A) の全期間honest再構築（quarterly walk-forwardモデル使用）。

rebuild_s7_walkforward_pg.py の7A版。VPS PG一本化（2026-07-22〜）に伴い、
環境変数をpopしないPG直読みの単発スクリプトとして実行する。

使い方:
    PYTHONPATH=. .venv/bin/python scripts/rebuild_s9a_walkforward_pg.py [--dry-run]
"""
...
from scripts.backfill_s9a_rank_wt import build_rows
...
def wipe_rows_pg(date_from: str, date_to: str, dry_run: bool) -> None:
    ...
    with get_connection() as conn:
        ...
        print(f"[rebuild-s9a-pg] 既存 #9A 行（{date_from}〜{date_to}）: {n}件 → 削除"
...
def insert_rows_pg(rows: list[dict], dry_run: bool) -> None:
    ...
    with get_connection() as conn:
        conn.executemany(
            ...
            rows_ins)
        conn.commit()
    print(f"[rebuild-s9a-pg] {len(rows)}件 書き込み完了（VPS PG）")
...
def main() -> None:
    ...
    for date_from, date_to, eval_model, win_model in quarters:
        print(f"\n[rebuild-s9a-pg] {date_from}〜{date_to}  eval={eval_model} win={win_model}", flush=True)
        ...
        print(f"[rebuild-s9a-pg]   9A: {len(rows)}R ({len(rows)/n_days:.1f}R/日) 的中{n_hit} "
    ...
    print(f"\n[rebuild-s9a-pg] ===== 全期間合計 =====")
    print(f"[rebuild-s9a-pg] 9A: {len(all_rows)}R 的中{total_hit} "
    ...
    if args.dry_run:
        print("[rebuild-s9a-pg] DRY RUN（書き込みなし）")
