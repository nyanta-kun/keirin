#!/usr/bin/env python3
"""7B(RANK_7B) の全期間honest再構築（月次凍結vintageモデル使用）。

rebuild_7a_walkforward_pg.py の7B版。VPS PG一本化（2026-07-22〜）に伴い、
...
使い方:
    PYTHONPATH=. .venv/bin/python scripts/rebuild_7b_walkforward_pg.py [--dry-run]
    PYTHONPATH=. .venv/bin/python scripts/rebuild_7b_walkforward_pg.py --skip-missing-models
"""
...
from scripts.backfill_7b_rank_wt import build_rows
...
_SCRIPT_NAME = "rebuild_7b_walkforward_pg.py"
...
def main() -> None:
    ...
    if missing:
        ...
        if not args.skip_missing_models:
            ...
            print(f"[rebuild-7b-pg] --skip-missing-models 未指定のため処理を中断します"
                  f"（計算は一切行っていません）。")
            sys.exit(1)
        notify_discord_warning(
        ...
        print(f"[rebuild-7b-pg] --skip-missing-models 指定のため{len(missing)}窓を"
              f"除外して続行します。")

    windows = available
    if not windows:
        print("[rebuild-7b-pg] 処理対象の窓がありません（全窓でモデル不足）。終了します。")
        sys.exit(1)
    ...
    for date_from, date_to, eval_model, win_model in windows:
        ...
        print(f"[rebuild-7b-pg]   7B: {len(rows)}R ({len(rows)/n_days:.1f}R/日) 的中{n_hit} "
    ...
    print(f"\n[rebuild-7b-pg] ===== 全期間合計 =====")
    print(f"[rebuild-7b-pg] 7B: {len(all_rows)}R 的中{total_hit} "
