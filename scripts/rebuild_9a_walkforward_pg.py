#!/usr/bin/env python3
"""9A(RANK_9A) の全期間honest再構築（quarterly walk-forwardモデル使用）。
...
使い方:
    PYTHONPATH=. .venv/bin/python scripts/rebuild_9a_walkforward_pg.py [--dry-run]
    PYTHONPATH=. .venv/bin/python scripts/rebuild_9a_walkforward_pg.py --skip-missing-models
"""
...
from scripts.backfill_9a_rank_wt import build_rows
...
_RANK_LABEL = "RANK_9A"
_DELETE_COND = "rank='RANK_9A' AND race_key LIKE '%#9A' AND race_date BETWEEN ? AND ?"
_SCRIPT_NAME = "rebuild_9a_walkforward_pg.py"
...
def main() -> None:
    ...
    if missing:
        ...
        if not args.skip_missing_models:
            ...
            print(f"[rebuild-9a-pg] --skip-missing-models 未指定のため処理を中断します"
                  f"（計算は一切行っていません）。")
            sys.exit(1)
        notify_discord_warning(
        ...
        print(f"[rebuild-9a-pg] --skip-missing-models 指定のため{len(missing)}窓を"
              f"除外して続行します。")

    windows = available
    if not windows:
        print("[rebuild-9a-pg] 処理対象の窓がありません（全窓でモデル不足）。終了します。")
        sys.exit(1)

    per_window_rows: list[tuple[str, str, list[dict]]] = []
    all_rows: list[dict] = []
    for date_from, date_to, eval_model, win_model in windows:
        print(f"\n[rebuild-9a-pg] {date_from}〜{date_to}  eval={eval_model} win={win_model}", flush=True)
        ...
        print(f"[rebuild-9a-pg]   9A: {len(rows)}R ({len(rows)/n_days:.1f}R/日) 的中{n_hit} "
    ...
    print(f"\n[rebuild-9a-pg] ===== 全期間合計 =====")
    print(f"[rebuild-9a-pg] 9A: {len(all_rows)}R 的中{total_hit} "
