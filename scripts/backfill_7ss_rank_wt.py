...
本スクリプトの `if len(board) != 7: continue`（盤面がちょうど7車でなければ
...
レース全体を候補プールから除外している（5点固定の買い目設計に合わせた簡略化）。
...
unify を見送った理由: 7SS(`sevenss_score >= SEVENSS_SCORE_THRESHOLD`)自体は
日次capを持たない独立per-race判定のため、S7本体（`s7_evening_reselect`の
日次capトリム）ほどの連鎖リスクはない。ただし backfill_*_rank_wt.py の実行
