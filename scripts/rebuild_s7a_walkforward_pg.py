#!/usr/bin/env python3
"""7A(SEVEN_7A) の全期間honest再構築（quarterly walk-forwardモデル使用）。
...
def wipe_rows_pg(date_from: str, date_to: str, dry_run: bool) -> None:
    ...
    with get_connection() as conn:
        ...
        print(f"[rebuild-s7a-pg] 既存 #7A 行（{date_from}〜{date_to}）: {n}件 → 削除"
...
def main() -> None:
    ...
    for date_from, date_to, eval_model, win_model in windows:
        ...
        print(f"[rebuild-s7a-pg]   7A: {len(rows)}R ({len(rows)/n_days:.1f}R/日) 的中{n_hit} "
    ...
    print(f"\n[rebuild-s7a-pg] ===== 全期間合計 =====")
    print(f"[rebuild-s7a-pg] 7A: {len(all_rows)}R 的中{total_hit} "
