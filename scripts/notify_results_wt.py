...
ランク体系（2026-07-17〜: 現行は S2/S3 の2ペーパーランクのみ）:
  ...
  S3(#7M)  = ◎不一致×システム◎×gap12≥0.10（内部rank 7PLUS_M・2026-07-17 新定義。
    旧定義の波乱ゲート entropy/mto は廃止）
...
def _main_inner(date, _db_url):
    ...
    # A（{rk}#A）・S1（{rk}#S1）の decisions 注入は 2026-07-17 全廃
    ...
    with get_connection() as conn:
        for (venue, race_no, _slot), (rank, ptime, combo_str) in sorted(picks.items(), key=lambda x: (x[0][0], x[0][1], x[0][2])):
            ...
            # 7plus_a / six_s1 スロットの採点は 2026-07-17 全廃（A・S1廃止）
    ...
    # S2/S3（ペーパー）は独立行で表示（ヘッダー合計には不算入）
    ...
    for _l in (u_line, m_line, r_line, ss_line, s_line):
        if _l:
    ...
    if rank_lines:
        msg += "\n" + "\n".join(rank_lines)
    msg += "\n```\n" + "\n".join(
        total_7plus + results_7plus_u + results_7plus_m) + "\n```"
