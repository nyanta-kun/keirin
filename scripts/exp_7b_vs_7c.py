...
def main():
    ...
    print("\n=== 重なったレース（同一母集団での直接比較）===")
    agg(common, b7, "7B の買い方（3点）")
    agg(common, c7, "7C の買い方（4-5点）")
    agg(common, c7, "7C ＋傾斜配分（本番）", tilt=True)

    print("\n=== それぞれの独自部分 ===")
    agg(sorted(set(b7) - set(c7)), b7, "7B のみ")
    agg(sorted(set(c7) - set(b7)), c7, "7C のみ")

    print("\n=== 全体 ===")
    agg(sorted(b7), b7, "7B 全体")
    agg(sorted(c7), c7, "7C 全体")
