#!/usr/bin/env python3
"""S4(SEVEN_S4)行を四半期対応のリークなしモデルで全期間再構築する（2026-07-21）。

S1/S3と同じ四半期ウォークフォワードモデル（eval_q24xx/win_q24xx等）で
SEVEN_S4行をリークなしで全期間再構築する。

読み取りは常にローカル完全SQLiteを使う（PG側wt_oddsは2026-06以降のみの
ミラーで全履歴を持たないため）。書き込みはwipe前に
scripts/rebuild_coverage_guard.py でローカルDBカバレッジを確認する
（2026-07-19発覚のS1/S3データ消失事故の再発防止）。

使い方:
    PYTHONPATH=. .venv/bin/python scripts/rebuild_s4_walkforward.py [--dry-run]
"""
...
from scripts.backfill_s4_rank_wt import build_rows, insert_rows, wipe_rows
...
# 常にローカル完全SQLiteを使う必要がある（rebuild_s1/s3_walkforward.pyと同じ対策）。
...
def main() -> None:
    ...
    for date_from, date_to, eval_model, win_model in quarters:
        print(f"\n[rebuild-s4] {date_from}〜{date_to}  eval={eval_model} win={win_model}", flush=True)
        ...
        print(f"[rebuild-s4]   S4: {len(rows)}R ({len(rows)/n_days:.1f}R/日) 的中{n_hit} "
    ...
    print(f"\n[rebuild-s4] ===== 全期間合計 =====")
    print(f"[rebuild-s4] S4: {len(all_rows)}R 的中{total_hit} "
    ...
    if args.dry_run:
        print("[rebuild-s4] DRY RUN（書き込みなし）")
