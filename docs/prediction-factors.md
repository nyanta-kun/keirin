...
**9S/9A の母集団で見るとROIも改善する**（軸2車ながし7点）:

| | n | 軸2車的中 | ROI | 的中時中央配当 |
|---|---|---|---|---|
| 現行 | 477 | 26.6% | 73.6% | 12.0倍 |
| 補正後 | 459 | **32.0%** | **97.5%** | 12.1倍 |

改善の主因は母集団の入れ替えである。**9Bという受け皿が無いため overlap が 1→2 に
動いた117レースは推奨外に落ちる**が、それらは軸2車的中18.8%・ROI31.7%と母集団で
最悪の帯だった（逆に流入する99レースは33.3%・109.4%と最良）。件数は477→459と
ほぼ維持される。月次11ヶ月中10ヶ月で改善し、最高配当5本を除いても 55.0→78.3% と
改善幅を保つ（9B却下時に問題となった「少数の大穴への依存」ではない）。

⚠️ **ROI改善の統計的有意性は確立していない**。母集団を固定した paired bootstrap では
+6.92%・95%CI [−0.51, +15.51] で ns（的中率 +2.50pt は有意）。改善の大半が母集団
入替効果で paired test にかけられないこと、n=459 が小さいこと、月次σが 40.4→41.2 と
大きいままであることに注意。**ROIは live 実測（picks_history）で追うこと。**

⚠️ **軸1（単独1位）の3着内率・1着的中率はいずれも有意差なし**。
1着を決め打つ買い目（三連単1着固定）へ転用してはいけない。

**7Bとの関係**: 7B は `wt_overlap_n==2` **かつ** `order_disagree`（1着モデルの最上位≠◎）
の両方が必須で、後者は overlap2 の約11%しか通らない。仮に7車へ適用した場合、
overlap が 1→2 へ動いたレースの**約89%は7Bにも入らず推奨外**になる。7車で適用しない
判断はこの点でも支持される。
