...
> **現行戦略（2026-07-16 再編・全ランクペーパー）**: S1=**6車三連単** モデル1位→2位→{3,4位} 2点・gap12≥0.11（3独立窓 ROI110/103/113%）、S2=波乱ライン連れ込み三連複（7車）、S3=◎不一致×システム◎三連複（7車）、A=◎一致×波乱×別ライン先頭軸の二連単・目5〜50倍（7車）。**旧S1（7車三連複・7PLUS_R・実賭け）は検証期間ROI67.3%・代替条件なしのため同日全廃**（過去行は picks_history_r_archive へ退避）。詳細は CLAUDE.md「現行ランク体系」。
