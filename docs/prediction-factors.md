...
> **現行戦略（2026-07-16 名称整理: SS→S1 / U→S2 / M→S3・A新設）**: **7車専用**。S1=三連複レース単位（min全目≥7∧gap12≥0.10∧gap23≥1pt∧非選抜・唯一の実賭け）、S2=波乱ライン連れ込み三連複（ペーパー）、S3=◎不一致×システム◎三連複（ペーパー）、A=◎一致×波乱×別ライン先頭軸の二連単・目5〜50倍（ペーパー）。詳細は CLAUDE.md「現行ランク体系」。
