...
> **現行戦略（2026-07-21 S2/S3全廃・S4をSS/Sへ再編・S1/SS/S の3ペーパーランク）**: S1=win軸1着固定×3着内モデル相手2車（7車・軸=1着専用モデル1位×top3_gap≥0.22×三連単2点流し・目下限なし・現状維持でブラッシュアップ方向を継続検討）。S2(旧U)・S3(旧M)は対象レース数・的中率・期待値の観点で継続困難と判断し2026-07-21に全廃（S2 honest全期間ROI84.8%・S3同120.4%だったが廃止対象、過去行は picks_history_u_archive / picks_history_m_archive へ退避）。S4(単勝×複勝指数トップ3重なり軸×波乱度選出の三連複2軸総流し・7車)は今後の予想データのベースと位置づけ、軸2車がWINTICKET公式◎◯と重なる数で**SS**（重なり0・全く重ならない・honest全期間ROI232.8%）と**S**（重なり1・片方だけ重なる・同120.6%）に再編（重なり2=完全一致は除外・内部rankは`SEVEN_S4`のまま`gate_label`列でSS/Sを区別）。**旧新S1（6車三連単）・A（一致波乱二連単）は検証ROI100%超なしのため 2026-07-17 全廃**（行は picks_history_r_archive / picks_history_a_archive へ退避）。旧S1（7車三連複・7PLUS_R・実賭け）は 2026-07-16 全廃済み。詳細は CLAUDE.md「現行ランク体系」。
