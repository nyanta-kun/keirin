...
**9S/9A の母集団で見ると ROI も改善する**（軸2車ながし7点・n=477→459）:
...
改善の主因は母集団の入れ替えで、**9Bという受け皿が無いため overlap が 1→2 に動いた
117レースは推奨外に落ちる**が、それらは軸2車的中18.8%・ROI31.7%と母集団で最悪の帯
だった（逆に流入する99レースは33.3%・109.4%と最良）。月次11ヶ月中10ヶ月で改善し、
最高配当5本を除いても 55.0→78.3% と改善幅を保つ（＝少数の大穴依存ではない）。

⚠️ ただし **ROI 改善の統計的有意性は確立していない**。母集団を固定した paired
bootstrap では +6.92%・95%CI [−0.51, +15.51] で ns（的中率 +2.50pt は有意）。
改善の大半が母集団入替効果で paired test にかけられないこと、n=459 が小さいこと、
月次σが 40.4→41.2 と大きいままであることに注意。ROI は live 実測で追うこと。
