...
# ═══════════════════════════════════════════════════════════════════════════
# 新S1（6車三連単・モデル1位→2位→{3位,4位} 2点）— 2026-07-16 ペーパートレード検証中
#
# 対象: 6車立て（n_entries==6）∧ gap12（モデル予測確率 1位−2位・rawスケール）
#       >= S1_GAP12_MIN。買い目は三連単 m1→m2→m3 / m1→m2→m4 の2点・100円/点。
# 検証（exp_6car_freeze.py・gap12≥0.11 凍結値・3独立窓）:
#   窓1 lgbm_wt_eval    2026-04-13〜07-15: n=148 的中19.6% ROI110.4%
#   窓2 lgbm_wt_eval_w2 2026-01-01〜04-12: n=164 的中15.2% ROI102.9%
#   窓3 lgbm_wt_eval_w3 2025-10-01〜12-31: n=137 的中20.4% ROI113.4%
# 約1.5R/日。ライブは記録のみ（ペーパー・賭けなし）で採否判定する。
# 閾値は凍結値（都度分位は使わない・変更禁止）。
# ═══════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════
# M（◎不一致×システム◎）戦略 — 2026-07-16 ペーパートレード検証中
#
# WT◎（prediction_mark==1）とシステム◎（モデル指数1位）が不一致の波乱見込み
# レースで、システム◎と同ライン脚質「逃」の相方を2車軸にした三連複流し
# （オッズ15倍以上の目のみ）。市場順位条件はなし（Uとの相違点）。
# 検証: exp_mismatch_m1_wt.py（テスト48R ROI121.8% / VAL 244R 120.9%）。
# entropy / mto / 買い目オッズの凍結閾値は U と同一値を再利用する
# （U_ENTROPY_MIN / U_MTO_MIN / U_LEG_MIN_ODDS）。
...
# ═══════════════════════════════════════════════════════════════════════════
# A（◎一致×波乱×別ライン先頭・二連単）戦略 — 2026-07-16 ペーパートレード検証中
#
# WT◎（prediction_mark==1）とシステム◎（モデル指数1位）が一致した波乱見込み
# レース（entropy≥U_ENTROPY_MIN・mto条件なし）で、◎と別ラインの「先頭」のうち
# 競走得点最上位の1車を軸にした二連単 軸→全流し（目オッズ 5〜50倍のみ）。
# ◎が1着のレース（一致◎の1着率47-48%）では自動的に外れる=「◎が勝たない52%」を
# 構造的に狙う。検証: kiseki scratchpad wt_match_hitrate_frontier.py ほか
# （窓1 ROI100.2%/的中16.9% / 窓2 96.7%/16.7%・約10R/日）。損益分岐圏のため
# ライブは記録のみ（ペーパー）で live 100R 以上で採否判定。
# entropy 閾値は U と同じ凍結値（U_ENTROPY_MIN）を再利用する。
# ═══════════════════════════════════════════════════════════════════════════
