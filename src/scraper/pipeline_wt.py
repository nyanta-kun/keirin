...
class WinticketPipeline:
    def collect_date(self, target_date: str, dry_run: bool = False) -> dict:
        ...
        keirin-station の races テーブルに当日開催記録があれば、その会場だけに
        絞り込んで無駄な探索リクエストを削減する（無ければ全会場を走査）。
        """
        stats = {"venues": 0, "races": 0, "results": 0, "errors": 0}
        venue_ids = _venues_racing_on(target_date) or list(VENUE_SLUGS.keys())
...
def _venues_racing_on(target_date: str) -> list[str]:
    """keirin-station の races テーブルから当日開催のある会場コードを取得。

    winticket でカバーされる会場（VENUE_SLUGS）に限定して返す。
    ks に記録が無い日は空リストを返し、呼び出し側で全会場走査にフォールバック。
    """
    try:
