...
## ★日次cron時刻 7:00→8:00 変更（2026-06-09・ユーザー判断）
朝7時は想定オッズが揃わずガミ3段階判定の精度が落ちる(夜レースで9999.9倍プレースホルダ等)ため8時算出に変更。`daily_picks_wt.sh`/docs更新済。crontab は手動更新要(本環境でcrontab書込ハングのため): `crontab -l|sed 's#0 7 \* \* \*.*daily_picks_wt#0 8 * * * /Users/ysuzuki/GitHub/keirin/scripts/daily_picks_wt.sh >> /Users/ysuzuki/GitHub/keirin/data/logs/cron.log 2>\&1#'|crontab -` 等。週次23:30・intraday(10-23)は変更なし。
