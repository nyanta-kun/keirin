...
- **A（2026-07-16 新設）** = `7PLUS_A`（suffix `#7A`・ペーパー）: ◎一致（WT◎=システム◎）∧ 指数エントロピー≥1.84（mto条件なし）× 軸=◎と別ラインの先頭のうち競走得点最上位 → **二連単** 軸→全のうち目5〜50倍のみ（`A_EX_MIN_ODDS`/`A_EX_MAX_ODDS`）。検証 窓1 100.2%/的中16.9%・窓2 96.7%/16.7%・約10R/日（的中率重視・損益分岐圏）。過去分は `scripts/backfill_a_rank_wt.py` で 2026-04-13〜を lgbm_wt_eval で遡及構築済み
