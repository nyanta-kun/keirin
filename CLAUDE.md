...
- **SS** = 内部rank `7PLUS_R`（race_key suffix `#7R`）: 三連複レース単位。min(全目オッズ)≥7.0 ∧ gap12≥0.10 ∧ gap23≥1pt ∧ **非選抜** → 全目購入 100円/点（唯一の実賭けランク）
  - ポリシーは選抜カットのみ。doc53 の 4分戦カット・ライン格差≥1.5増額は 2026-07-16 に削除（実精算再検証で窓間方向不一致・`exp_ss_policy_realistic_wt.py`）
- **U** = `7PLUS_U`（suffix `#7U`・**ペーパートレード＝賭けなし記録のみ**）: 波乱見込み（指数エントロピー≥1.84 ∧ 盤面min三連複≥4.3・凍結値）× 穴（市場4-7位∧モデル3位内∧ライン先頭/番手）× 同ライン「逃」相方の三連複2車軸流し・目≥15倍のみ。検証 テスト110.9%/VAL118.9%
- **M** = `7PLUS_M`（suffix `#7M`・ペーパー）: WT◎≠システム◎の波乱見込みレース × システム◎ × 同ライン「逃」相方・目≥15倍。検証 121.8%/120.9%。U(buy)と同一ペアなら U 優先で M は記録しない
- U/M は 2026-08末に live 100R以上で採否判定（月次判定は分散的に禁物・40R全外れ月も想定内）。詳細はメモリ `keirin_u_rank_paper`
...
- 見送り=miwokuri=TRUE。**実賭け集計は `rank='7PLUS_R' AND NOT miwokuri AND bet_amount>0` のみ**（U/M は集計不算入・ヘッダー合計にも含めない）
- `prerace_decisions_{date}.json` が採点/Web/サマリー/Discord の正本（15分前判定を事後変更しない）。U/M は `{rk}#U` / `{rk}#M` キー
