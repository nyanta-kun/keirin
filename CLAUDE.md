...
## 現行ランク体系（2026-07-16〜・実精算方式・**全ランクペーパートレード**）

**2026-07-16 名称整理: SS→S1 / U→S2 / M→S3・A 新設**、**同日 旧S1（7PLUS_R・7車三連複・唯一の実賭け）を全廃し新S1（6車三連単）へ置換**。
**2026-07-16 指数改定: 競走得点トレンド4特徴を追加（FEATURE_COLS_WT 40→44・全モデル再学習・バックフィル再構築済み）**

- **S1（6車三連単・新）** = 内部rank `SIX_S1`（race_key suffix `#6S1`・decisions キー `{rk}#S1`・**ペーパー**）: **6車立て ∧ gap12≥0.11**（`S1_GAP12_MIN` 凍結値）→ 三連単 モデル1位→2位→{3位,4位} の2点・100円/点。検証 3独立窓 ROI 110.4/102.9/113.4%・的中15-21%・約1.5R/日（`exp_6car_freeze.py`）。エッジは着順指定にのみ存在（三連複だと60%台）＝6車三連単の流動性の薄さ由来。過去分は `scripts/backfill_s1_six_wt.py`
- **旧S1（7PLUS_R・7車三連複・全廃）**: 検証期間ROI 67.3%・代替条件の全探索（3窓18条件・6/9車・三連単高配当）で黒字なしのため 2026-07-16 全廃。過去行（7PLUS_R/7PLUS_CAND/7PLUS_SS/7PLUS_S）は `picks_history_r_archive` へ退避。wave-picks の SS txtセクション・#CAND 書き込み・ガミ判定は停止済み（ss_policy 等は互換のため残置）
- **S2（旧U）** = `7PLUS_U`（suffix `#7U`・**ペーパートレード＝賭けなし記録のみ**）: 波乱見込み（指数エントロピー≥1.84 ∧ 盤面min三連複≥4.3・凍結値）× 穴（市場4-7位∧モデル3位内∧ライン先頭/番手）× 同ライン「逃」相方の三連複2車軸流し・目≥15倍のみ。検証 テスト110.9%/VAL118.9%
- **S3（旧M）** = `7PLUS_M`（suffix `#7M`・ペーパー）: WT◎≠システム◎の波乱見込みレース × システム◎ × 同ライン「逃」相方・目≥15倍。検証 121.8%/120.9%。S2(buy)と同一ペアなら S2 優先で S3 は記録しない
- **A（2026-07-16 新設）** = `7PLUS_A`（suffix `#7A`・ペーパー）: ◎一致（WT◎=システム◎）∧ 指数エントロピー≥1.84（mto条件なし）× 軸=◎と別ラインの先頭のうち競走得点最上位 → **二連単** 軸→全のうち目5〜50倍のみ（`A_EX_MIN_ODDS`/`A_EX_MAX_ODDS`）。約9R/日（的中率重視・損益分岐圏。44特徴指数での検証期間 93.2%/的中15.7%）。過去分は `scripts/backfill_a_rank_wt.py` で 2026-04-13〜を lgbm_wt_eval で遡及構築済み
  - **旧・廃止済みAランク（買い目カット方式・〜2026-06-19）の 7PLUS_A 行は `picks_history_a_archive` へ退避済み**（新Aと race_key 衝突するため）
- S1/S2/S3/A は live 100R以上で採否判定（実賭け昇格 or 廃止。月次判定は分散的に禁物・40R全外れ月も想定内）。詳細はメモリ `keirin_u_rank_paper` / `keirin_s1_redesign_sweep`
...
- 見送り=miwokuri=TRUE。**実賭けランクは現在なし**（全ランクペーパー・名目賭金）。Webサマリーのトップラインは `rank IN ('SIX_S1','7PLUS_U','7PLUS_M','7PLUS_A')` の名目合算
- `prerace_decisions_{date}.json` が採点/Web/サマリー/Discord の正本（15分前判定を事後変更しない）。キーは S1=`{rk}#S1` / S2=`{rk}#U` / S3=`{rk}#M` / A=`{rk}#A`
