...
## 現行ランク体系（2026-07-21〜・実精算方式・**S1/S2/S3/S4 の4ペーパーランク**）
...
- **S4（単勝×複勝指数トップ3重なり軸×波乱度選出・2026-07-21導入）** = `SEVEN_S4`（suffix `#7S4`・**ペーパートレード**）: 軸2車 = `pred_win_pct`（単勝指数）上位3 ∩ `pred_top3_pct`（複勝指数）上位3 の重なり車から選定（重なり>=2なら`pred_top3_pct`上位2、重なり==1ならその1車+残りの`pred_top3_pct`最上位。重なり0は対象外・実データで58,616中1件のみ）。波乱度指数 = 軸2車の`pred_top3_pct`合計（`axis_sum`）。**レース全体のエントロピー（拮抗度）で絞るとROIが悪化する（絞り込みなし85.7%→73.5%）ことを確認し不採用**、axis_sum昇順（軸自体が弱い＝波乱寄り）で当日該当レースを日次ランキングし上位`S4_DAILY_TOP_N`=15件を採用。買い目 = 三連複 軸2車+残り5車のいずれか1車の5点（オッズ下限なし）。S1/S3と同じ四半期walk-forwardモデルで検証時点からリークなしのhonest全期間実績（2024-01-01〜2026-07-10）= 13,830R・約15R/日・的中36.7%・ROI**111.7%**（9四半期中8四半期がROI100%超）。N/日を5〜30で振ると検証窓・テスト窓とも単調減衰（181.5→97.4%検証・153.4→101.0%テスト）で自然な閾値として信頼できる。単勝指数側の信号（win_max・単勝トップ2合計）との複合も検証したが改善なし（複勝指数トップ2合計と相関が強く追加情報量が乏しい）。過去分は `scripts/rebuild_s4_walkforward.py`（旧`backfill_s4_rank_wt.py --wipe`は単一モデル版で最新期間のみ）
...
- **S3（2026-07-17 新定義・2026-07-19 3way OR拡張・同日中にモデルリーク発覚→修正）** = `7PLUS_M`（suffix `#7M`・ペーパー）: **WT◎≠システム◎（不一致）** ∧ 以下いずれか（軸信頼ゲート）: (a) gap12≥0.10（`M_GAP12_MIN`）(b) システム◎の1着モデル(`lgbm_wt_win`)内順位≥3（`M_WIN_RANK_MIN`・Phase B 2026-07-19導入）(c) システム◎の p_win/p_top3 比≤0.30（`M_RATIO_MAX`・(b)の連続量版・2026-07-19導入）。× システム◎ × 同ライン「逃」相方の三連複2車軸流し・目≥15倍のみ。**旧定義の波乱ゲート（entropy≥1.84∧mto≥4.3）は廃止**。正規プロトコル（`exp_composite_prob_diff_wt.py`は独自に時系列分割した専用モデルで評価・当初からリークなし）: gap12単独 検証111.8%(221R)→テスト104.4%(62R) → gap12 OR win_rank 検証158.2%(531R)→テスト149.5%(152R) → gap12 OR win_rank OR ratio 検証158.6%(671R)→テスト154.3%(186R)。**【2026-07-19同日中に発覚した重大な訂正】** picks_historyの過去再構築（本番モデル`lgbm_wt_win`はfull_refit=Trueでホールドアウトなし＝毎回全期間で再学習）は、gap12側の`lgbm_wt_eval_q24xx`等が四半期ウォークフォワードモデルなのに対しwin側だけ対応モデルがなく非対称で、過去レースを「未来を知るモデル」でスコアリングするリークがあった（`exp_composite_prob_diff_wt.py`のテスト154.3%と実際のpicks_history集計89.0%が乖離したことで発覚）。`lgbm_wt_win_q24xx`等10モデルを新規学習し`scripts/rebuild_s3_walkforward.py`で全期間honest再構築した結果: **全期間ROI95.9%(3114R)**・ゲート別内訳 win_rank**119.1%**(736R・最強)／gap12 87.9%(1900R)／ratio 88.2%(478R)。S2(buy)と同一ペアなら S2 優先で S3 は記録しない。過去分は `scripts/rebuild_s3_walkforward.py`（旧`backfill_um_rank_wt.py --wipe-m`は同型リークのため非推奨・詳細はメモリ`keirin_composite_ratio_gate`）
...
- S1/S2/S3/S4 は live 100R以上で採否判定（実賭け昇格 or 廃止。月次判定は分散的に禁物・40R全外れ月も想定内）。詳細はメモリ `keirin_u_rank_paper` / `keirin_s1_redesign_sweep` / `keirin_s1_win_axis_paper` / `keirin_picks_history_data_loss_2026_07_20`（S4）
...
- `prerace_decisions_{date}.json` が採点/Web/サマリー/Discord の正本（15分前判定を事後変更しない）。キーは S1=`{rk}#S1` / S2=`{rk}#U` / S3=`{rk}#M` / S4=`{rk}#S4`
