...
## 現行ランク体系（2026-07-16〜・7車専用・実精算方式）

**2026-07-16 名称整理: SS→S1 / U→S2 / M→S3・A 新設**（内部rankコード・suffixは不変。表示名のみ変更）
**2026-07-16 指数改定: 競走得点トレンド4特徴を追加（FEATURE_COLS_WT 40→44・全モデル再学習・バックフィル再構築済み）**。S1 は検証期間 ROI 67.3% と低迷しており条件見直し検討中（ROI追及・少数精鋭・S2/S3と非重複の方向）

- **S1（旧SS）** = 内部rank `7PLUS_R`（race_key suffix `#7R`）: 三連複レース単位。min(全目オッズ)≥7.0 ∧ gap12≥0.10 ∧ gap23≥1pt ∧ **非選抜** → 全目購入 100円/点（唯一の実賭けランク）
  - ポリシーは選抜カットのみ。doc53 の 4分戦カット・ライン格差≥1.5増額は 2026-07-16 に削除（実精算再検証で窓間方向不一致・`exp_ss_policy_realistic_wt.py`）
...
- S2/S3/A は live 100R以上で採否判定（S2/S3は2026-08末目安。月次判定は分散的に禁物・40R全外れ月も想定内）。詳細はメモリ `keirin_u_rank_paper`
...
- 見送り=miwokuri=TRUE。**実賭け集計は `rank='7PLUS_R' AND NOT miwokuri AND bet_amount>0` のみ**（S2/S3/A は集計不算入・ヘッダー合計にも含めない）
- `prerace_decisions_{date}.json` が採点/Web/サマリー/Discord の正本（15分前判定を事後変更しない）。S2/S3/A は `{rk}#U` / `{rk}#M` / `{rk}#A` キー
